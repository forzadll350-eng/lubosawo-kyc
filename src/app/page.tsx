...
export default function LandingPage() {
  ...
  function mapAuthErrorMessage(errorLike: { message?: string; status?: number } | null | undefined): string {
    ...
    if (
      ...
      return "ส่งอีเมลถี่เกินไป ระบบจำกัดชั่วคราว กรุณารอสักครู่แล้วลองใหม่";
    }
    if (
      status === 504 ||
      m === "{}" ||
      ...
      return "ส่งอีเมลยืนยันไม่สำเร็จ (SMTP timeout) กรุณาตรวจ SMTP ใน Supabase: Host/Port/Username/Password และ Sender email";
    }
    if (m.includes("invalid login credentials")) {
      return "อีเมลหรือรหัสผ่านไม่ถูกต้อง หากเคยสมัครอีเมลเดิมไว้แล้วให้กดลืมรหัสผ่าน";
    }

    return raw || "เกิดข้อผิดพลาดในการยืนยันตัวตน กรุณาลองใหม่อีกครั้ง";
  }
  ...
  const inputCls =
    "w-full px-3.5 py-2.5 border-[1.5px] border-gray-200 rounded-md text-sm focus:border-navy-3 focus:ring-2 focus:ring-navy-3/10 outline-none transition-all";
    ...
    <div className="flex min-h-screen">
      <div className="hidden lg:flex w-[52%] bg-gradient-to-br from-navy via-navy-2 to-navy-3 p-16 flex-col justify-between relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 relative z-10">
          <div className="w-14 h-14 bg-gradient-to-br from-gold to-gold-2 rounded-xl flex items-center justify-center text-navy font-bold text-lg shadow-gold">
            ลบส
          </div>
          <div>
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-4xl font-bold text-white leading-tight mb-4">
            พิสูจน์ตัวตน
            <br />
            อย่าง<span className="text-gold-2">น่าเชื่อถือ</span>
          </h2>
          <p className="text-white/60 text-sm leading-relaxed max-w-md">
            ระบบยืนยันตัวตนผ่านบัตรประชาชนและการจดจำใบหน้า เพื่อการเข้าใช้บริการระบบสารบรรณอิเล็กทรอนิกส์อย่างปลอดภัย
          </p>
          ...
          <div className="mt-10 flex flex-col gap-4">
            ...
            <div className="flex items-start gap-3.5">
              <div className="w-10 h-10 bg-white/5 border border-white/10 rounded-lg flex items-center justify-center text-lg shrink-0">📸</div>
              <div>
                <strong className="text-white text-sm block">Selfie + Liveness Check</strong>
                <span className="text-white/50 text-xs">ถ่ายรูปใบหน้าเพื่อเปรียบเทียบกับบัตร</span>
              </div>
            </div>
            <div className="flex items-start gap-3.5">
              <div className="w-10 h-10 bg-white/5 border border-white/10 rounded-lg flex items-center justify-center text-lg shrink-0">✅</div>
              <div>
                <strong className="text-white text-sm block">ตรวจสอบโดยเจ้าหน้าที่</strong>
                <span className="text-white/50 text-xs">ทีมงานตรวจสอบและอนุมัติภายใน 1-2 วัน</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 bg-white flex items-center justify-center p-6 lg:p-10">
        <div className="w-full max-w-[420px]">
          <div className="flex gap-1 bg-gray-100 rounded-lg p-1 mb-8">
            {[
              { k: "login", l: "เข้าสู่ระบบ" },
              { k: "register", l: "สมัครสมาชิก" },
              ...
              <button
                ...
                className={
                  ...
                  (activeTab === t.k ? "bg-white text-navy shadow-sm" : "text-gray-500 hover:text-gray-700")
                }
              >
                {t.l}
              </button>
          ...
          {error && (
            <div className="mb-4 p-3 bg-status-red-light border border-status-red/30 rounded-lg text-status-red text-xs font-semibold">
              {error}
            </div>
          )}
            ...
            <form onSubmit={handleLogin} className="animate-fade-up">
              ...
              <div className="mb-6">
                ...
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 text-sm">
                    {showPw ? "🙈" : "👁️"}
                  </button>
                </div>
              </div>
              ...
              <button type="button" onClick={handleForgotPassword} disabled={resetSending} className="w-full mt-2 py-2.5 border border-gray-200 text-gray-600 rounded-md text-xs font-semibold hover:border-navy-3 hover:text-navy-3 transition-all disabled:opacity-60">
                {resetSending ? "กำลังส่งลิงก์..." : "ลืมรหัสผ่าน"}
              </button>

              <p className="text-center text-xs text-gray-400 mt-4">
                ยังไม่มีบัญชี? <button type="button" onClick={() => setActiveTab("register")} className="text-navy-3 font-bold hover:underline">สมัครสมาชิก</button>
              </p>
            </form>
          )}
